...
# One step build and test for the ios binaries
# All resulting binaries for ios are in mobilewallet/ilxd_bridge/ios
...
RUST_TARGET_ARCH=aarch64-apple-ios
ARCH=arm64
...
# Copy libillium_zk.a and libillium_crypto.a into our bridge's ios/ folder
...
clang -c ilxd_zk_bridge.m -o "ilxd_zk_bridge_${OS_NAME}_${ARCH}.o" \
    ...
    -fobjc-arc \
    -fmodules \
    -miphoneos-version-min=${IOS_VERSION_MIN}
...
clang -dynamiclib -o "libilxd_zk_bridge_${OS_NAME}_${ARCH}.dylib" \
    ...
    ${ILXD_HOME}/zk/rust/target/aarch64-apple-ios/release/libillium_zk.a \
    -arch ${ARCH} \
    -isysroot "${IOS_SDK_PATH}" \
    -fobjc-arc \
    -fmodules \
...
clang -dynamiclib -o "libilxd_crypto_bridge_${OS_NAME}_${ARCH}.dylib" \
    ...
    ${ILXD_HOME}/crypto/rust/target/aarch64-apple-ios/release/libillium_crypto.a \
    -arch ${ARCH} \
    -isysroot "${IOS_SDK_PATH}" \
    -fobjc-arc \
    -fmodules \
...
if [ ! -f "ios/libilxd_zk_bridge_${OS_NAME}_${ARCH}.dylib" ]; then
    echo "Error: libilxd_zk_bridge_${OS_NAME}_${ARCH}.dylib not found"
    exit 1
fi

if [ ! -f "ios/libilxd_crypto_bridge_${OS_NAME}_${ARCH}.dylib" ]; then
    echo "Error: libilxd_crypto_bridge_${OS_NAME}_${ARCH}.dylib not found"
    exit 1
fi

echo "ilxd_bridge/ios:"
ls -l ios/**
echo ""

# Now build the dart component
dart pub get
dart run test/test_ios.dart
set +x
